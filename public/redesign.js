...
intro.innerHTML = '<div><span>STOCK BASKET</span><strong>8</strong><p>Tokenized stocks in the current strategy</p></div><div><span>CAMPAIGN INCENTIVE</span><strong>10 <small>% APR</small></strong><p>For eligible holdings; campaign terms apply</p></div><div><span>STARTING AMOUNT</span><strong>25 <small>USDC</small></strong><p>Minimum amount to open a portfolio</p></div><div><span>STOCK REWARDS DISTRIBUTED</span><strong>$2.1K</strong><p>Worth of stock rewards distributed to date</p></div>';
...
panels.rewards.querySelector('.workspace-heading p:last-child').textContent = 'The BaseStock10 token is live on Base. Over $2,100 worth of 10-stock rewards has been sent to date.';
document.querySelector('.rewards-teaser .soon-pill').textContent = 'TOKEN LIVE ON BASE';
document.querySelector('.rewards-teaser p').textContent = 'BaseStock10 is live on Base. Over $2,100 worth of 10-stock rewards has been sent to date.';
...
document.querySelector('.buy-launch').innerHTML = `<p>Eligible BaseStock10 holders can receive ten-stock rewards.</p><small>Over $2,100 worth of 10-stock rewards has been sent to date. Eligibility and distribution rules can change.</small><a class="contract-link" href="${baseStockTokenUrl}" target="_blank" rel="noreferrer">VIEW CONTRACT ↗</a><a class="buy-token" href="https://kyberswap.com/swap/base/usdc-to-0x51276d0322421b9a903132b3867bcda2587cfee1" target="_blank" rel="noreferrer">BUY BASESTOCK10 ↗</a>`;
intro.lastElementChild.innerHTML = '<span>10-STOCK REWARDS SENT</span><strong>$2,100</strong><p>Worth of stock rewards sent to date</p>';
hero.querySelector('.network').insertAdjacentHTML('afterend', '<div class="hero-reward-callout"><span>10-STOCK REWARDS SENT</span><strong>$2,100</strong><p>Worth of stock rewards sent to date</p></div>');
routeHash();
})();
